...
function turtlebot_control(bot1Name, bot2Name, bot3Name)
    ...
    while (true)
        ...
        for i = 1:3
            for j = 1:3
                costMatrix(i,j) = norm(events(i,:) - robotPoses(j,:));
            end
        end
        
        % Use Hungarian algorithm to assign events uniquely to robots
        % match: Nx2 array where match(i,1)=event, match(i,2)=robot
        match = matchpairs(costMatrix, 9999999999);
        
        % Fill assignments: event_assignments(i) = robot assigned to event i
